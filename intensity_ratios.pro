...
fname ='/data/iris_l2_20150206_051507_3800256196_raster_t000_r00000.fits'
...
thick = ''
READ, thick, PROMPT='Write?'
IF thick EQ 'y' OR thick EQ 'Y' THEN BEGIN
	;opt_thick = [[opt_thick], [coords]]
	ID = STRING(FORMAT='(I03)', K)
	scrncap=TVRD(TRUE=1)
	WRITE_PNG, path + '/analysis/' + obs_tag + '/images/RATIO_' +ID+ '_img.png', scrncap
...
;OPLOT, (wav_trunc2-wav_shift), (img2-im2_err), COLOR=80
	scrncap=TVRD(TRUE=1)
	WRITE_PNG, path + '/analysis/' + obs_tag + '/images/LINES_' +ID+ '_img.png', scrncap
ENDIF ELSE BEGIN
